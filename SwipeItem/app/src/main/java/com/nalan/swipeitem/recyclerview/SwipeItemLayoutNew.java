...
public class SwipeItemLayoutNew extends ViewGroup {
    public static final String TAG = "SwipeItemLayoutNew";
    enum Mode{
    ...
    public SwipeItemLayoutNew(Context context) {
        this(context,null);
    }

    public SwipeItemLayoutNew(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        Log.e(TAG,"Group_onInterceptTouchEvent"+action);
        switch (action) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        final int action = ev.getActionMasked();
        Log.e(TAG,"Group_onTouchEvent"+action);
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                final int x = (int) ev.getX();
                final int y = (int) ev.getY();
    ...
    private class ScrollRunnable implements Runnable{
        ...
        void startScroll(int startX,int endX){
            if(startX!=endX){
                ...
                ViewCompat.postOnAnimation(SwipeItemLayoutNew.this,this);
            }
        }
    ...
    public static class OnSwipeItemTouchListener implements RecyclerView.OnItemTouchListener {
        private SwipeItemLayoutNew captureItem;
        ...
        public boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent ev) {
            Log.e(TAG,"List_onInterceptTouchEvent"+"_"+probingParentProcess);
            ...
            if(action!=MotionEvent.ACTION_DOWN && ignoreActions){
                Log.e(TAG,"action1_"+true);
                return true;
            }
            Log.e(TAG,"captureItem="+captureItem);
            if(action!=MotionEvent.ACTION_DOWN && (captureItem==null||parentHandled)){
                Log.e(TAG,"ACTION2_"+false);
                return false;
            }
            ...
            switch (action){
                case MotionEvent.ACTION_DOWN:{
                    ...
                    SwipeItemLayoutNew pointItem = null;
                    //首先知道ev针对的是哪个item
                    View pointView = findTopChildUnder(rv,(int)x,(int)y);
                    Log.e(TAG,"POINT_VIEW_"+pointView);
                    if(pointView==null || !(pointView instanceof SwipeItemLayoutNew)){
                        //可能是head view或bottom view
                        if (!(pointView instanceof LinearLayout)){
                            pointOther = true;
                        }else {
                            pointItem = (SwipeItemLayoutNew) ((LinearLayout) pointView).getChildAt(1);
                        }
                    }else
                        pointItem = (SwipeItemLayoutNew) pointView;
                    Log.e(TAG,"POINT1111="+pointItem);
                    ...
                    if(!pointOther){
                        ...
                        Log.e(TAG,"pointOther11111111+"+false);
                        //如果它在fling，就转为drag
                        ...
                        Log.e(TAG,"DISALLOW_"+disallowIntercept);
                        ...
                        //直接将其close掉
                        Log.e(TAG,"pointOther2222222+"+true);
                        if(captureItem !=null &&
                                captureItem.getScrollOffset()!=0) {
                            captureItem.close();
                            ignoreActions = true;
                            Log.e(TAG,"captureItem_"+true);
                            return true;
                        }

                        captureItem = null;
                        Log.e(TAG,"POINT22222="+pointItem);
                        if(pointItem!=null) {
        ...
        public void onTouchEvent(RecyclerView rv, MotionEvent ev) {
            Log.e(TAG,"List_onTouchEvent"+"_"+ignoreActions);
            if(ignoreActions)
                return;
