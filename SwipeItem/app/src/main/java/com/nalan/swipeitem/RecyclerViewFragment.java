...
import com.nalan.swipeitem.ipc.TestActivity;
import com.nalan.swipeitem.recyclerview.SwipeItemLayoutNew;
...
public class RecyclerViewFragment extends Fragment{
    ...
    public static RecyclerViewFragment getInstance(int type){
        ...
        bundle.putInt("RecyclerViewFragment",type);
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mAnInt = arguments.getInt("RecyclerViewFragment", 0);
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if(root==null){
            ...
            recyclerView.addOnItemTouchListener(new SwipeItemLayoutNew.OnSwipeItemTouchListener(getContext()));
    ...
    public class MyAdapter extends RecyclerView.Adapter<MyAdapter.Holder> {
        ...
        public Holder onCreateViewHolder(ViewGroup parent, int viewType) {
            View root;
            if (mAnInt==1){
                 root = LayoutInflater.from(mContext).inflate(R.layout.item_recycler, parent, false);
            }else {
                root = LayoutInflater.from(mContext).inflate(R.layout.item_recycler_test,parent,false);
            }
        ...
        class Holder extends RecyclerView.ViewHolder implements View.OnClickListener, View.OnLongClickListener {
                    ...
                    case R.id.main:
                        ...
                        new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                int workerId = 50;
                                int other = 100;
                                List<SaleCase> saleCaseList = getBySaleId(workerId);
                                List<VirtualCase> virtualCaseList = getByVirtualId(workerId);
                                for (SaleCase saleCase : saleCaseList) {
                                    addIntoAdjust();
                                }
                                for (VirtualCase caseInfo : virtualCaseList) {
                                    addIntoAdjust();
                                }
                                RespDao resp = new RespDao();
                                List<Map<String,Object>> toWorkerList = new ArrayList<>();
                                for (Integer i : workList) {
                                    Map<String,Object> m = new HashMap<>();
                                    if (i>20){
                                        resp.setId(i);
                                        m.put("workerId",i);
                                        m.put("workerName","TestWorker+"+i);
                                        toWorkerList.add(m);
                                    }
                                }
                                resp.setToWorkerIdList(toWorkerList);
                                long end = System.currentTimeMillis();
                                Log.e("TimeAll","time="+(end-start));
                            }
                        }).start();
            ...
            private List<VirtualCase> getByVirtualId(int workerId) {
                List<VirtualCase> list = new ArrayList<>();
                for (int i = 0; i < 1000; i++) {
                    VirtualCase saleCase = new VirtualCase();
                    saleCase.setVirtualId(i);
                    saleCase.setVirtualName("商品名称"+i);
                    saleCase.setVirtualDes("哈哈建行卡回到户籍放开化肥肯定是开发商疯狂建设局开发商开发和"+i);
                    list.add(saleCase);
                }
                List<VirtualCase> tempList = new ArrayList<>();
                for (VirtualCase saleCase : list) {
                    if (saleCase.getVirtualId()>workerId){
                        tempList.add(saleCase);
                    }
                }
                return tempList;
            }

            private List<SaleCase> getBySaleId(int workerId) {
                List<SaleCase> list = new ArrayList<>();
                for (int i = 0; i < 1000; i++) {
                    SaleCase saleCase = new SaleCase();
                    saleCase.setSaleId(i);
                    saleCase.setSaleName("商品名称"+i);
                    saleCase.setSaleDes("哈哈建行卡回到户籍放开化肥肯定是开发商疯狂建设局开发商开发和"+i);
                    list.add(saleCase);
                }
                List<SaleCase> tempList = new ArrayList<>();
                for (SaleCase saleCase : list) {
                    if (saleCase.getSaleId()>workerId){
                        tempList.add(saleCase);
                    }
                }
                return tempList;
    ...
    class SolutionBean {
        public List<List<Integer>> threeSum(int[] nums) {
            List<List<Integer>> solutList = new ArrayList<>();
            Arrays.sort(nums);
            for (int i = 0; i < nums.length-2; i++) {
                ...
                for (int j = i+1; j < nums.length; j++) {
                    ...
                    if (map.get(temp) != null) {
                        ...
                        tempList.add(nums[i]);
                        tempList.add(nums[map.get(temp)]);
                        tempList.add(nums[j]);
                        if (!solutList.contains(tempList)){
                            solutList.add(tempList);
                        }
                        break;
                    } else {
                        map.put(nums[j], j);
        ...
        public int[] intersect(int[] nums1, int[] nums2) {
            ...
            for (int i : nums1) {
                if (record.get(i)==null){
                    ...
                    Integer integer = record.get(i);
                    record.put(i,++integer);
                }
            }
            List<Integer> temp = new ArrayList<>();
            for (int i = 0; i < nums2.length; i++) {
                if (record.get(nums2[i])!=null && record.get(nums2[i])>0){
                    temp.add(nums2[i]);
                    int integer = record.get(nums2[i]);
                    record.put(nums2[i],--integer);
                }
            }
        ...
        public int reverse(int x) {
            ...
            try {
                if (result.endsWith("0")){
                    String st = result.substring(0, result.lastIndexOf('0'));
                    return isDown?-Integer.valueOf(st):Integer.valueOf(st);
                }else {
                    return isDown?-Integer.valueOf(result):Integer.valueOf(result);
                }
            }catch (Exception e){
                return 0;
            }
    ...
    public static class Test16{
        ...
        public static void main(String[] args) {
            ...
            if (b){
                ...
                System.out.println("end="+threeSum(tempArr));
            }
        }


        public static List<List<Integer>> threeSum(int[] nums) {
            Set<List<Integer>> solutList = new HashSet<>();
            if(nums.length<3){
                return new ArrayList<>(solutList);
            }
            Arrays.sort(nums);
            for (int i = 0; i < nums.length; i++) {
                if (nums[i]>0){
...
//            }
    ...
    class Solution13 {
        ...
        private void count(Map<String,Integer> map,String arr){
          if (map.get(arr)!=null){
              Integer integer = map.get(arr);
              map.put(arr,++integer);
          }else {
              map.put(arr,1);
          }
    ...
    private void setMapData(Map<String, Integer> map, String s, String tag) {
        int count = s.length() - s.replace(tag, "").length();
        if (map.get(tag)!=null){
